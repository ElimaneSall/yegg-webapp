...
type RestOf<T extends IFeedback | NewFeedback> = Omit<T, 'dateCreation'> & {
...
export class FeedbackService {
  ...
  protected convertDateFromClient<T extends IFeedback | NewFeedback | PartialUpdateFeedback>(feedback: T): RestOf<T> {
    return {
      ...feedback,
      dateCreation: feedback.dateCreation?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restFeedback: RestFeedback): IFeedback {
    return {
      ...restFeedback,
      dateCreation: restFeedback.dateCreation ? dayjs(restFeedback.dateCreation) : undefined,
    };
  }
