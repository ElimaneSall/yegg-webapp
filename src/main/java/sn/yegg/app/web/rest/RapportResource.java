...
import sn.yegg.app.service.RapportQueryService;
...
/**
 * REST controller for managing {@link sn.yegg.app.domain.Rapport}.
 */
@RestController
...
public class RapportResource {
    ...
    private final RapportQueryService rapportQueryService;

    public RapportResource(RapportService rapportService, RapportRepository rapportRepository, RapportQueryService rapportQueryService) {
        this.rapportService = rapportService;
        this.rapportRepository = rapportRepository;
        this.rapportQueryService = rapportQueryService;
    }
    ...
    public ResponseEntity<List<RapportDTO>> getAllRapports(
        RapportCriteria criteria,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to get Rapports by criteria: {}", criteria);

        Page<RapportDTO> page = rapportQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /rapports/count} : count all the rapports.
     *
     * @param criteria the criteria which the requested entities should match.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the count in body.
     */
    @GetMapping("/count")
    public ResponseEntity<Long> countRapports(RapportCriteria criteria) {
        LOG.debug("REST request to count Rapports by criteria: {}", criteria);
        return ResponseEntity.ok().body(rapportQueryService.countByCriteria(criteria));
    }
